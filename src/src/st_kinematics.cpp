...
CarState STKinematics::update(const CarState start, double accel,
    ...
    double slip_angle_dot =
        (first_term) *
            (p.cs_f * start.steer_angle * (rear_val)-start.slip_angle *
             ...
             vel_ratio *
                 (p.cs_r * p.l_r * (front_val)-p.cs_f * p.l_f * (rear_val))) -
...
CarState STKinematics::update_k(const CarState start, double accel,
                                double steer_angle_vel, CarParams p,
                                double dt) {
    ...
    double steer_angle_dot = steer_angle_vel;
    double theta_dot =
        start.velocity / p.wheelbase * std::tan(start.steer_angle);
    double theta_double_dot =
...
CarState STKinematics::update_with_pacejka(const CarState start, double accel,
