...
std::vector<double> Precompute::get_car_distances(int scan_beams, double wheelbase, double width, 
    ...
    std::vector<double> car_distances(scan_beams);  // 벡터의 크기를 미리 설정
    ...
    for (int i = 0; i < scan_beams; i++) {
        ...
        if (angle > 0) {
            if (angle < PI / 2.0) {
                ...
                // car_distances.push_back(std::min(to_side, to_front));
            } else {
                // between pi/2 and pi
                ...
                // car_distances.push_back(std::min(to_side, to_back));
            } 
        } else {
            if (angle > -PI / 2.0) {
                // between 0 and -pi/2
...
std::vector<double> Precompute::get_cosines(int scan_beams, double angle_min, double scan_ang_incr) {
    // Precompute distance from lidar to edge of car for each beam
    std::vector<double> cosines(scan_beams);  // 벡터의 크기를 미리 설정
